...
router.post('/get', (req, res, next) => {
    ...
    if (id) {
        ...
        Artical.count().then((count) => {
            pages = count / limit;
            counts = count;
            Artical.find().sort({_id: -1}).limit(limit).skip(skip).populate(['catagory']).then((articals) => {
                responseData.code = 0;
                responseData.message = '获取成功';
...
router.post('/getByCatagory', (req, res, next) => {
    ...
    if (catagoryId) {
        Artical.count({catagoryId: catagoryId}).then((count) => {
            ...
            Artical.find({catagory: catagoryId}).sort({_id: -1}).limit(limit).skip(skip).populate(['catagory']).then((articals) => {
                responseData.code = 0;
                responseData.message = '获取成功';
                responseData.articals = articals;
                responseData.count = counts;
